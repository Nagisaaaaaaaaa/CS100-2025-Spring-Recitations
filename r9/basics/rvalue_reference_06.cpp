...
void Go(std::string &&str) {
  ...
  //! 这行代码的意义：
  //!   借助“掠夺”和延长右值生命周期，这套组合拳，我们没有做任何拷贝。
}
...
int main() {
  ...
  //
  // 调用：
  //   Go(std::string("QAQ")); //* 纯右值 (prvalue)
  //   Go(std::move(str));     //* 将亡值 (xvalue)
  //
  //* 输入的右值，每一层函数调用，都会被“掠夺”一次。
  //* 讲得好听点，每一层函数调用，都会被“移动”一次。
  //* 这就是 std::move 这个名字的由来。
