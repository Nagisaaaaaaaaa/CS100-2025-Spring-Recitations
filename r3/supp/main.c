/// \file
/// \note Don't worry, you are not forced to understand the mathematics.
/// Just type the code and enjoy the beautiful pictures.
/// We hope you will like it.
///
/// \brief This file visualizes the Mandelbrot set.
/// See https://en.wikipedia.org/wiki/Mandelbrot_set.
/// The most common algorithm used to compute the Mandelbrot set is the "escape time algorithm".
...
/// \brief "Dwell" is the fancy name for the maximum number of iterations on one number.
/// Since the actual "dwell"s might be very large and difficult for numerical computation,
/// they are always clamped to `maxDwell`.
///
/// So, the value of "dwell" determines whether the point is in the set, which
/// obeys the following rules:
/// 1. If "dwell" equals `maxDwell`, the point belongs to the set.
...
int Dwell(int i, int j, int width, int height, double cMinRe, double cMinIm, double cMaxRe, double cMaxIm) {
  ...
  int dwell = 0;
  while ((dwell < maxDwell) && (zRe * zRe + zIm * zIm < 2.0 * 2.0)) {
